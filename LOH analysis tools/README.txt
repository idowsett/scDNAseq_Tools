This set of scripts produces, analyzes and graphs LOH events that occured in a yeast lineage using four input files generated by the snakemake pipeline included in the Mutator Volatility Snakemaker Pipeline.

The four files generated are in the noSNP.var file format and correspond to: an Aged Mother Yeast's last daughter, that daughter's daughter 
and a corresponding "forever young" lineage pair that have undergone the same number of divisions.
...
These locations can then be visualized on the graphical yeast genome using the script: LOHgraphing.py.
